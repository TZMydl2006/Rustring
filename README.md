...
本次更新在原有全站搜索基础上新增了三个核心增强：

### 搜索模式选择
- 搜索框下方增加了三个单选按钮：`All`、`Title`、`Tags`。
- **All**：默认模式，全文搜索（标题、标题头、标签、摘要、正文均参与匹配和评分）。
- **Title**：仅匹配页面标题字段，适合快速定位特定文档。
- **Tags**：仅匹配页面标签字段，适合按标签分类查找。
- 切换模式后搜索会立即重新执行，无需重新构建。

### 匹配片段高亮
- 搜索结果摘要中匹配的关键词会以 `<mark>` 标签包裹，在浏览器中呈现黄色背景（或其他可配置颜色）。
- 帮助用户一眼识别关键词在文档中的位置，区分“真匹配”与“假匹配”。

### 匹配类型标签
- 每个搜索结果卡片会显示一个最高优先级的匹配类型标签（如“标题匹配”、“标签匹配”、“摘要匹配”、“正文匹配”），说明该结果被命中的主要原因。
- 优先级顺序：标题 > 标题头（heading） > 标签 > 摘要 > 正文。

### 高亮颜色选择器
- 搜索面板新增了一组颜色圆形按钮（黄、绿、蓝、粉、橙、默认主题色）。
- 用户点击任意颜色，所有搜索结果中的 `<mark>` 高亮会立即切换为所选颜色。
- 颜色选择保存在浏览器本地存储中（`localStorage`），刷新页面或重启服务器后保持。
- 可根据当前主题（白天/黑夜/系统）或个人视觉偏好自由调整。

### 与功能直接相关的代码更新
- `src/render.rs`：
  - `MAIN_TEMPLATE` 与 `ARCHIVE_TEMPLATE` 中的搜索面板新增了模式选择器（`<div class="search-mode">`）和颜色选择器（`<div class="highlight-color">`）。
...
切换模式后搜索立即生效，适合不同场景下的查找需求。

### 匹配片段高亮

搜索结果的摘要中，与关键词匹配的部分会被 `<mark>` 标签高亮（默认黄色背景）。您可以通过颜色选择器自定义高亮颜色。

### 匹配类型标签

每个搜索结果下方会显示一个匹配类型标签（如“标题匹配”、“标签匹配”），说明该结果被命中的主要原因。这有助于判断结果是否真正符合预期。

### 自定义高亮颜色

在搜索面板的“Highlight”区域，点击颜色圆形按钮可以改变高亮颜色。可选颜色包括：
- 黄色（默认）
- 绿色
- 蓝色
- 粉色
